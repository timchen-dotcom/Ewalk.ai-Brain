...
重置前必做清單：[[2026-06-05_MacStudio重置前必做清單]]
...
- 舊主機常駐自動化來源需確認，避免重置後排程或提醒消失。
- Mac Studio 完整備份需完成並抽查可讀。
...
```text
新筆電可接手日常阿順工作入口。
Mac Studio 繼續保留，不可重置，直到舊釘選 thread 完整歷史、常駐自動化來源與完整備份完成收尾。
```
...
- 新筆電已完整接手。
- Ewalk.ai Brain 文字資料已推送 GitHub 遠端。
- 剩餘素材、成品檔與本機設定已有備份策略。
- Mac Studio 重要資料已有新筆電或外部備份。
- 舊 Codex 釘選 thread 已輸出交接摘要。
- 本機常駐自動化與排程已確認可停用、搬移或重建。
- 私密憑證已確認可重新取得或已進入安全憑證流程。
- 提姆先生確認可以重置。
- 不需要保留個人資料。
- OpenClaw 主機安裝計畫已確認。
