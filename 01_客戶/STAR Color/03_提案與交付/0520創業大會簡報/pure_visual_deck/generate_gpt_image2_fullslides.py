...
DEFAULT_PROMPTS = ROOT / "gpt_image2_fullslide_prompts.json"
DEFAULT_OUT = ROOT / "fullslide_images_1536_confirm"
DEFAULT_BRAND_KIT = ROOT.parents[2] / "01_品牌資料" / "簡報生成品牌包" / "brand_kit.json"
...
def post_json(url: str, api_key: str, payload: dict, timeout: int = 240, max_attempts: int = 5) -> dict:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            last_error = f"OpenAI Images API error {exc.code}: {body}"
            ...
            print(
                f"[retry] transient API error {exc.code}; attempt {attempt}/{max_attempts}, "
                f"waiting {wait_seconds}s",
                file=sys.stderr,
            )
            time.sleep(wait_seconds)
        except urllib.error.URLError as exc:
            last_error = f"OpenAI Images API network error: {exc}"
            if attempt == max_attempts:
                raise
            wait_seconds = min(90, 8 * attempt)
            print(
                f"[retry] network error; attempt {attempt}/{max_attempts}, waiting {wait_seconds}s",
                file=sys.stderr,
            )
            time.sleep(wait_seconds)
...
def build_prompt(spec: dict, page: dict) -> str:
    brand_kit = load_brand_kit(spec)
    brand_guardrails = "\n".join(f"- {rule}" for rule in brand_kit.get("image_generation_prompt_guardrails", []))
    global_rules = "\n".join(f"- {rule}" for rule in spec.get("global_rules", []))
    must_text = "\n".join(f"- {text}" for text in page.get("must_include_text", []))
    return (
        "Create one finished full-slide presentation image.\n\n"
        f"Deck: {spec.get('deck_title', '')}\n"
        f"Page {int(page['page']):02d} role: {page.get('role', '')}\n\n"
        f"Brand kit guardrails:\n{brand_guardrails}\n\n"
        f"Brand style:\n{spec.get('brand_style', '')}\n\n"
        f"Global rules:\n{global_rules}\n\n"
        f"Main headline:\n{page.get('headline', '')}\n\n"
        f"Supporting idea:\n{page.get('supporting_text', '')}\n\n"
        f"Exact visible text to include:\n{must_text}\n\n"
        f"Visual scene:\n{page.get('visual_scene', '')}\n\n"
        f"Chart or structure:\n{page.get('chart_or_structure', '')}\n\n"
        f"Page-specific instruction:\n{page.get('prompt', '')}\n\n"
        f"Negative requirements:\n{spec.get('negative', '')}\n\n"
        "Final quality bar: a premium conference keynote slide, readable Traditional Chinese typography, "
        "clear hierarchy, no extra text beyond the requested labels."
    )


def load_brand_kit(spec: dict) -> dict:
    raw_path = spec.get("brand_kit_path")
    path = Path(raw_path).expanduser() if raw_path else DEFAULT_BRAND_KIT
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate full baked STAR Color slides with OpenAI GPT Image 2.")
    ...
    parser.add_argument("--force", action="store_true", help="Overwrite existing fullslide_images/page_XX.png")
